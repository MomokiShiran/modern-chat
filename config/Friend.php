...
class Friend {
    ...
    public function getFriends($user_id) {
        try {
            $stmt = $this->conn->prepare(
                "SELECT u.id as friend_id, u.username, u.email, u.avatar, u.status, f.created_at 
                 FROM friends f 
            ...
            foreach ($friends as &$friend) {
                $friend['friend_id'] = $friend['friend_id'] ?? $friend['id'] ?? 0;
                ...
                $stmt->execute([$user_id, $friend['friend_id'], $friend['friend_id'], $user_id]);
                $last_message = $stmt->fetch();
                
                if ($last_message) {
                    $friend['last_message'] = $last_message['content'];
                    $friend['last_message_time'] = $last_message['created_at'];
                    $friend['is_me'] = $last_message['sender_id'] == $user_id;
                } else {
                    $friend['last_message'] = '';
